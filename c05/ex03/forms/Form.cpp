...
std::string Form::getName(const Bureaucrat & rhs) const
{
	...
	{
		std::cerr << ERR << rhs.getName() << " cannot get name of contract because exec grade is too high" << DEF << std::endl;
		return ("unknown");
	}
}
...
void Form::beSigned(const Bureaucrat & rhs) 
{
	...
	{
		std::cerr << ERR << rhs.getName() << " cannot sign because the grade is too high" << DEF << std::endl; 
	}
}
